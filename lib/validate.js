...
export function validatePackage(input, { themes: allowedThemes, statuses }) {
  ...
  // Places you'll see — [{ name, image }]; drop rows missing a name or a valid image
  const places = (Array.isArray(input.places) ? input.places : [])
    .map((p) => ({
      name: str(p?.name).slice(0, 80),
      image: str(p?.image),
      publicId: str(p?.publicId) || null,
    }))
    .filter((p) => p.name && /^https:\/\//.test(p.image))
    .slice(0, 12);

  // Map location — optional plain place string
  const mapQuery = str(input.mapQuery).slice(0, 160) || null;
