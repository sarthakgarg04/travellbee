...
export default function MobileMenu() {
  ...
  // Portals need the DOM, so only render the overlay after the client mounts.
  ...
  // If the viewport grows to desktop, close the drawer so it never lingers on lg+.
  useEffect(() => {
  ...
  // The overlay is portaled to <body> so it escapes the header's blur/opacity
  // context. That is what makes the panel fully opaque and layered correctly.
  const overlay = (
    <div className="lg:hidden">
      ...
      <div
        onClick={close}
        aria-hidden="true"
        className={`fixed inset-0 z-[100] bg-black/60 transition-opacity duration-300 ease-out motion-reduce:transition-none ${
      ...
      <div
        ...
        aria-modal="true"
        aria-label="Menu"
        className={`fixed top-0 right-0 z-[101] h-full w-[84%] max-w-sm bg-white dark:bg-darkbg shadow-2xl flex flex-col will-change-transform transition-transform duration-300 ease-out motion-reduce:transition-none ${
